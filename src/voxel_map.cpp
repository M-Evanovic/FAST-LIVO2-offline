...
void loadVoxelConfig(std::string config_path, VoxelMapConfig &voxel_config) {
  auto yaml = YAML::LoadFile(config_path);

  voxel_config.max_layer_ = yaml["lio"]["max_layer"].as<int>();
  voxel_config.max_voxel_size_ = yaml["lio"]["voxel_size"].as<double>();
  voxel_config.planner_threshold_ = yaml["lio"]["min_eigen_value"].as<double>();
  voxel_config.sigma_num_ = yaml["lio"]["sigma_num"].as<double>();
  voxel_config.beam_err_ = yaml["lio"]["beam_err"].as<double>();
  voxel_config.dept_err_ = yaml["lio"]["dept_err"].as<double>();
  voxel_config.layer_init_num_ = yaml["lio"]["layer_init_num"].as<std::vector<int>>();
  voxel_config.max_points_num_ = yaml["lio"]["max_points_num"].as<int>();
  voxel_config.max_iterations_ = yaml["lio"]["max_iterations"].as<int>();

  voxel_config.map_sliding_en = yaml["local_map"]["map_sliding_en"].as<bool>();
  voxel_config.half_map_size = yaml["local_map"]["half_map_size"].as<int>();
  voxel_config.sliding_thresh = yaml["local_map"]["sliding_thresh"].as<double>();
}
